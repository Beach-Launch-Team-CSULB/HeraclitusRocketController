...
void loop() {
    ...
    if(currentTime >= nextGUIReportTime)
    {
        ...
        {
            comSys.sendSensorData(SENS_1_4_PROP, max(0.0f, myRocket.sensorRead(PT_LOX_HIGH_ID)), max(0.0f, myRocket.sensorRead(PT_FUEL_HIGH_ID)), max(0.0f, myRocket.sensorRead(PT_FUEL_DOME_ID)), max(0.0f, myRocket.sensorRead(PT_FUEL_DOME_ID)));
            comSys.sendSensorData(SENS_5_8_PROP, max(0.0f, myRocket.sensorRead(PT_LOX_TANK_1_ID)), max(0.0f, myRocket.sensorRead(PT_LOX_TANK_2_ID)), max(0.0f, myRocket.sensorRead(PT_FUEL_TANK_1_ID)), max(0.0f, myRocket.sensorRead(PT_FUEL_TANK_2_ID)));
        }
