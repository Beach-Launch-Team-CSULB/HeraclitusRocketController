...
std::vector<Color> firstLED{YELLOW,  PINK, ORANGE, ORANGE, ORANGE, WHITE, ORANGE, GREEN};
std::vector<Color> secondLED{BLACK, BLACK, RED, GREEN,  BLUE,   WHITE, ORANGE, GREEN};
...
void executeCommand(uint32_t commandID) {   
    if (commandID == PING_PI_ROCKET) {
        ...
        if (commandID != VENT || commandID != ABORT) allOfTheLights.setLed(LED1, secondLED[commandID]);
